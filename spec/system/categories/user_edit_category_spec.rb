...
describe 'User creates a category', type: :system do
  ...
  let(:product) { create(:product, restaurant:, category:, name: 'Pizza de Calabresa') }
  ...
  context 'successfully' do
    it 'and see info in real time' do
    ...
    it 'and update category name in product associated' do
      fill_in 'Nome', with: 'Pizzas'
      fill_in 'Descrição', with: 'Categorias de pizzas diversas do nosso restaurante'
      click_button 'Salvar'
      ...
      within find('.category-card', text: 'Pizzas') do
        expect(page).to have_content('Pizzas')
        expect(page).to have_content("Produtos\n1")
        expect(page).to have_content("Ativos\n1")
        expect(page).to have_content("Preço Médio\nR$ 49,90")
      ...
      within '.tab' do
        find('span', text: 'Produtos').click
      end
      within find('.product-card', text: 'Pizza de Calabresa') do
        expect(page).to have_css('.product-stamp', text: 'Pizzas')
      end
    end
  end
